...
public class BadPacketsCheck {
    ...
    private static CheckResult checkAnimate(Player player){
        ...
        if((System.currentTimeMillis() - lastAnimate) < CheckType.BADPACKETS.otherData.getInt("animateDelay")){
    ...
    private static CheckResult checkAction(Player player,PlayerActionPacket packet){
        ...
        if(packet.action==PlayerActionPacket.ACTION_JUMP){
            if(!(checkCache.movementCache.topSolid && checkCache.movementCache.bottomSolid)&&(System.currentTimeMillis()-checkCache.lastJump)<CheckType.BADPACKETS.otherData.getInt("jumpCoolDown")){
