...
//                return new CheckResult("tried to fly on the Y-axis (mY=" + movementCache.motionY + ", max=" + maxMotionY + ")");
        ...
        if (!movementCache.onGround && movementCache.motionY < 0
            ...
            double gravitatedY = (movementCache.lastMotionY - 0.08) * CheckType.FLIGHT.otherData.getSection("gravity").getDouble("friction");
            double offset = Math.abs(gravitatedY - movementCache.motionY);
            double maxOffset = CheckType.FLIGHT.otherData.getSection("gravity").getDouble("maxOffset");
