...
public class SpeedCheck {
    public static CheckResult checkXZSpeed(Player player, double x, double z, Location movingTowards) {
        ...
        if (movementCache.airTicks > 1 && movementCache.elytraEffectTicks <= 0 &&
            ...
            double limit = CheckType.SPEED.otherData.getSection("airSpeed").getDouble("baseLimit");
            // jump may cause false result
            if((System.currentTimeMillis()-cache.lastJump)<500)
                limit += CheckType.SPEED.otherData.getSection("airSpeed").getDouble("jumpMultiplier");
            ...
            double walkSpeedMultiplier = CheckType.SPEED.otherData.getSection("airSpeed").getDouble("walkSpeedMultiplier");
            // 1.4
    ...
    private static double getMaxAcceptableMotionY(Player player, boolean nearBed, boolean onground, boolean halfMovement) {
        double base = (nearBed ? 0.6625 : ((halfMovement) ? 0.7 : CheckType.SPEED.otherData.getSection("vertical").getDouble("baseLimit")));

        if (!onground && MoveUtil.isNearSolid(player.getPosition()))
            base += CheckType.SPEED.otherData.getSection("vertical").getDouble("climbableCompensation");
