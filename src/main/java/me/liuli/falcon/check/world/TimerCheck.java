...
public class TimerCheck {
    public static CheckResult runCheck(Player player) {
        if (Server.getInstance().getTicksPerSecond() < CheckType.TIMER.otherData.getLong("minimumTps"))
        ...
        packetBalance += 50;
        packetBalance -= rate;
        int triggerBalance = CheckType.TIMER.otherData.getInt("triggerBalance");
        int minimumClamp = CheckType.TIMER.otherData.getInt("minimumClamp");
    ...
    public static void compensate(Player player) {
        ...
        cache.packetBalance -= CheckType.TIMER.otherData.getInt("teleportCompensation");
    }
}
